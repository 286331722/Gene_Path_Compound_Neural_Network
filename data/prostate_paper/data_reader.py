...
cnv_filename = 'breast_data_cna.csv'
#response_filename = 'response_paper.csv'
response_filename = 'breast_response.csv'#'breast_response.csv'
#gene_important_mutations_only = 'P1000_final_analysis_set_cross_important_only.csv'
gene_important_mutations_only = 'breast_data_mutations.csv'
promoter_filename = 'breast_data_promoters_rrbs.csv'
...
def load_data_type(data_type='gene', cnv_levels=5, cnv_filter_single_event=True, mut_binary=False, selected_genes=None):
    ...
    #add by qian for promoters_rrbs
    if data_type == 'promoter':
        x, response, info, genes = load_data(promoter_filename, selected_genes)
        #x[x <= 0.0] = 0.
        #x[x > 0.0] = 1.
    if data_type == 'cnv':
