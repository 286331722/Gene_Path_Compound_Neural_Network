...
class CrossvalidationPipeline(OneSplitPipeline):
    ...
    def run(self, n_splits=5):
        ...
        for data_params in self.data_params:
            ...
            for model_param in self.model_params:
                ...
                scores = self.train_predict_crossvalidation(m_param, X, y, info, cols, model_name,x_test_,y_test_,info_test_)
    ...
    def train_predict_crossvalidation(self, model_params, X, y, info, cols, model_name,x_test_,y_test_,info_test_):
        ...
        for train_index, test_index in skf.split(X, y.ravel()):
            ...
            x_train, x_test = X[train_index], x_test_
            y_train, y_test = y[train_index], y_test_
            info_train = pd.DataFrame(index=info[train_index])
            #info_test = pd.DataFrame(index=info[test_index])
            info_test = pd.DataFrame(index=info_test_)
