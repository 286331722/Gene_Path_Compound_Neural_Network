...
def get_validation_brca(cols, cnv_split):
    current_dir = dirname(dirname(realpath(__file__)))
    validation_data_dir = join(current_dir, '_database/prostate/external_validation/')

    valid_cnv = pd.read_csv(join(validation_data_dir, 'brca_mbcp_2022/data_cna_processed.txt'), index_col=0)
    valid_mut = pd.read_csv(join(validation_data_dir, 'brca_mbcp_2022/data_mutations_processed.csv'), index_col=0)


    # validation_data_dir = join(current_dir, '_database/prostate/')
    #
    # valid_cnv = pd.read_csv(join(validation_data_dir, 'processed/breast_data_cna.csv'), index_col=0)
    # valid_mut = pd.read_csv(join(validation_data_dir, 'processed/breast_data_mutations.csv'), index_col=0)
    ...
    #labels = pd.read_csv(join(validation_data_dir, 'splits/test_val_set.csv'))
    labels = pd.read_csv(join(validation_data_dir, 'brca_mbcp_2022/response_grade.csv'))
    labels = labels.set_index('id')

    valid_mut_df = valid_mut_df.join(labels, how='inner')
    valid_mut_df = valid_mut_df[~valid_mut_df['response'].isnull()]
    valid_cnv_df = valid_cnv_df.join(labels, how='inner')
    valid_cnv_df = valid_cnv_df[~valid_cnv_df['response'].isnull()]

    del valid_mut_df['response']
    del valid_cnv_df['response']


    valid_cnv_ampl = valid_cnv_df.copy()
    valid_cnv_ampl[valid_cnv_ampl <= 0.0] = 0.
    #valid_cnv_ampl[valid_cnv_ampl > 0.0] = 1.0
    valid_cnv_ampl[valid_cnv_ampl == 1.0] = 0.5
    valid_cnv_ampl[valid_cnv_ampl == 2.0] = 1.0


    valid_cnv_del = valid_cnv_df.copy()
    valid_cnv_del[valid_cnv_del >= 0.0] = 0.
    #valid_cnv_del[valid_cnv_del < 0.0] = 1.0
    valid_cnv_del[valid_cnv_del == -1.0] = 0.5
    valid_cnv_del[valid_cnv_del == -2.0] = 1.0

    valid_mut_df[valid_mut_df > 1.] = 1.

    df_list = [valid_cnv_ampl, valid_cnv_del,valid_mut_df]
    data_type_list = ['gene_final', 'cnv','']
    all_data = pd.concat(df_list, keys=data_type_list, join='inner', axis=1, )
    all_data = all_data.swaplevel(i=0, j=1, axis=1)
    ...
    cols = all_data.columns
    rows = pd.DataFrame(index=all_data.index)
    lables = labels.reindex(all_data.index)
    y = lables.values
    return x, y, rows, cols

def get_validation_tcga_negative(cols, cnv_split):
    current_dir = dirname(dirname(realpath(__file__)))
    #current_dir = dirname(dirname(realpath(__file__)))
    validation_data_dir = join(current_dir, '_database/prostate/external_validation/')

    valid_cnv = pd.read_csv(join(validation_data_dir, 'brca_tcga/data_cna_processed.txt'), index_col=0)
    valid_mut = pd.read_csv(join(validation_data_dir, 'brca_tcga/data_mutations_processed.csv'), index_col=0)
    ...
    labels = pd.read_csv(join(validation_data_dir, 'brca_tcga/her0.txt'))
    labels = labels.set_index('id')
...
class TrainValidatePipeline:
    ...
    def run(self):
        ...
        # 3- New dataset ()
        #x_test_mets, y_test_mets, info_test_mets, cols_test_mets = get_validation_tcga_pisitive(cols_train, cnv_split)
        #x_test_primary, y_test_primary, info_test_primary, cols_test_primary = get_validation_tcga_negative(cols_train,
           #                                                                                           cnv_split)
        # x_test_new, y_test_new, info_test_new, cols_test_new = get_validation_primary(cols_train)
        x_test_mets, y_test_mets, info_test_mets, cols_test_mets = get_validation_brca(cols_train, cnv_split)
        ...
        if type(self.model_params) == list:
            for m in self.model_params:
                ...
                def predict(x_test, y_test, info_test, model_name, test_set_name):
                    ...
                    y_pred_test_scores = (y_pred_test_scores1 + y_pred_test_scores1) / 2.
                    y_pred_test = y_pred_test_scores > 0.80#0.50#0.49
