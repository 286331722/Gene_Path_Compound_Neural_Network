...
def get_layer_maps_KEGG(genes):
    ...
    df_cmp_out = pd.DataFrame(index=cmps, columns=['out'])
    df_cmp_out[df_cmp_out != 1] = 1


    df_gen_path_temp = pd.read_csv(file_path_gen, header=None, index_col=0,sep="\t")
    row_cols = []
    for i, r in df_gen_path_temp.iterrows():
        col = i
        row = r.get(1)
        if row in df_gene_path.index and col in df_gene_path.columns:
            row_cols.append([row,col])
        #if row in df_gene_path.index and col in df_gene_path.columns:
    for row_col in row_cols:
        df_gene_path.loc[row_col[0],row_col[1]]=1.0

    df_path_cmp_temp = pd.read_csv(file_path_cmp, header=None, index_col=0,sep="\t")
    row_cols = []
    for i, r in df_path_cmp_temp.iterrows():
        row = i
        col = r.get(1)
        if row in df_path_cmp.index and col in df_path_cmp.columns:
            row_cols.append([row, col])
        #if row in df_path_cmp.index and col in df_path_cmp.columns:
    for row_col in row_cols:
        df_path_cmp.loc[row_col[0], row_col[1]] = 1.0

    maps = []

    maps.append(df_gene_path)
    maps.append(df_path_cmp)
    maps.append(df_cmp_out)
    print 'gene_path_map', df_gene_path.shape
    print 'path_cmp_map', df_path_cmp.shape
    print 'df_cmp_out',df_cmp_out.shape
...
def get_pnet(inputs, features, genes, n_hidden_layers, direction, activation, activation_decision, w_reg,
    ...
    if n_hidden_layers > 0:
        ...
        maps = get_layer_maps_KEGG(genes)
        ...
        dropouts = dropout[1:]
        for i, mapp in enumerate(maps[0:-1]):# modify by qian -1]):
            w_reg = w_regs[i]
