...
def get_predictions(filename, correct_prediction=True):
    ...
    if correct_prediction:
        df.pred = df.pred_scores >= 0.4#0.5
...
def get_clinical():
    filename = join(base_dir, 'prostate/supporting_data/data_clinical_patient.txt')
...
def plot_surv(ax, filename, correct_prediction):
    ...
    correct_full = clinical_df.merge(correct, how='inner', left_on='PATIENT_ID', right_index=True)
    wrong_full = clinical_df.merge(wrong, how='inner', left_on='PATIENT_ID', right_index=True)

    wrong_full = wrong_full.dropna(subset=['OS_MONTHS', 'OS_STATUS'])
    correct_full = correct_full.dropna(subset=['OS_MONTHS', 'OS_STATUS'])
    ...
    ax.set_xticks([-50,0,150,250,300])
