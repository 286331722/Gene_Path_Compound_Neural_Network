...
all_models_dict['GPC'] = df_pnet
#n = len(models) + 1

pnet_base_dir = join(base_dir, 'dense/onsplit_dense')
df_pnet = pd.read_csv(join(pnet_base_dir, 'P-net_ALL_testing.csv'), sep=',', index_col=0, header=[0, 1])
all_models_dict['Dense'] = df_pnet
n = len(models) + 2
...
def run_external_auc():
    fig, ax = plt.subplots(nrows=1, ncols=1, sharey=True, figsize=(5, 4), dpi=400)
    pnet_base_dir = join(base_dir, 'external_validation/pnet_validation')
    df = pd.read_csv(join(pnet_base_dir, 'P-net__mets_testing.csv'), sep=',', index_col=0, header=[0, 1])
    y_test = df['y']
    y_pred_score = df['score']
    fpr, tpr, thresholds = metrics.roc_curve(y_test, y_pred_score, pos_label=1)
    average_auc = metrics.auc(fpr, tpr)
    average_auc = average_precision_score(y_test, y_pred_score)
    colors = sns.hls_palette(n, l=.4, s=.7)
    #plot_roc(ax, y_test, y_pred_score, None, color=colors[1],label='external validation')

    fpr, tpr, thresholds = metrics.roc_curve(y_test, y_pred_score, pos_label=1)
    roc_auc = metrics.auc(fpr, tpr)

    ax.plot(fpr, tpr, label=' (area = %0.2f)' % roc_auc, linewidth=2, color=colors[0])
    ax.plot([0, 1], [0, 1], 'k--', alpha=0.1)
    ax.legend(loc=4)
    ax.set_xlim([0.0, 1.0])
    ax.set_ylim([0.0, 1.05])
    ...
    #plt.show()
    filename = join(saving_dir, 'external_auc')
    plt.savefig(filename, dpi=400)
    plt.savefig(filename + 'external.pdf', transparent=True)
    sns.set_style(None)
