...
def plot_compaison(ax1, label, df_pnet, df_dense):
    ...
    ax1.legend(['GPC', 'Dense'], fontsize=fontsize, loc='upper left', framealpha=0)
...
def plot_pnet_vs_dense_same_arch(ax):
    ...
    plot_roc(ax, y_test, y_pred_score, None, color=colors[0], label='GPC (156K params)')

    y_test = df_dense['y']
    y_pred_score = df_dense['pred_scores']
    plot_roc(ax, y_test, y_pred_score, None, color=colors[1], label='Dense (7.4M params)')
