...
def plot_high_genes2(ax, layer=1, graph='hist', direction='h'):
    ...
    if layer == 1:
        high_nodes = node_importance[node_importance.layer == layer].nlargest(20, columns=[column])
    else:
        high_nodes = node_importance[node_importance.layer == layer].nlargest(10, columns=[column])
    features = list(high_nodes.index)
    ...
    if graph == 'hist':
        ...
        if layer == 1:
            ax.set_ylabel('Gene'.format(layer), fontproperties, labelpad=10)
        if layer == 2:
            ax.set_ylabel('Pathway'.format(layer), fontproperties, labelpad=10)
        if layer == 3:
            ax.set_ylabel('Compound'.format(layer), fontproperties, labelpad=10)
...
def run():
    ...
    spec2 = gridspec.GridSpec(ncols=1, nrows=3, figure=fig,height_ratios=[2,1,1])
