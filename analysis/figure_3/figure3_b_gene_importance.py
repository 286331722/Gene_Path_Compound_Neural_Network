...
#     #     high_nodes.to_csv('../extracted/layer_high_{}.csv'.format(l))
...
#         plot_high_genes_histogram(df, features, y, name=str(l), saving_directory='./')
#         plot_high_genes_violinplot(df, features, y, name=str(l), saving_directory='./')
...
def plot_high_genes2(ax, layer=1, graph='boxplot_custom', direction='h'):
    if layer == 1:
...
def run():
    ...
    for l in layers:
        ...
        if l == 1:
            # high_nodes = node_importance[node_importance.layer == l].abs().nlargest(10, columns=['coef_combined'])
            high_nodes = node_importance[node_importance.layer == l].nlargest(10, columns=['coef'])
            plot_high_genes_sns(high_nodes, col='coef_combined', name=str(l), saving_directory=saving_directory)
            #plot_high_genes(high_nodes, col='coef_combined', name=str(l), saving_directory=saving_directory)
