...
def get_first_layer_df(nlargest):
    ...
    genes_weights = genes_weights[['coef']]
...
def get_x_y(df_encoded, layers_nodes):
    ...
    ind = node_weights.layer == 3
    node_weights.loc[ind,'x' ] = 0.2
    ind = node_weights.layer==4
    node_weights.loc[ind,'x' ] = 0.25
...
def get_fromated_network(links, high_nodes_df, col_name, remove_others):
    ...
    node_colors_dict['methylation'] = (155, 89, 110, 0.7)  # mutation
    ...
    #data = np.column_stack((node_code, node_colors_list, all_node_labels))
...
def run():
    ...
    #high_nodes_df.rename(columns={"promoter":"methylation"})
